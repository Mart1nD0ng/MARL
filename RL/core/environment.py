...
class CoreEnv:
    ...
    def step(self, deltaE: DeltaE, partitions: Dict[int, int]) -> Tuple[Dict[int, Observation], float, Dict]:
        ...
        # ==============================
        # Reward v2: Relative Utility Function (数学改进)
        # ==============================
        # 解决梯度消失问题：当success→1时，使用相对效用确保梯度信号持续存在
        ...
        # 1. 能量效率 (success per unit energy) - 核心优化目标
        # 避免分别惩罚success和energy导致的Pareto冲突
        energy_efficiency = float(consensus_success) / max(0.01, float(energy_cost) + 0.01)
        # log效率防止数值爆炸，同时确保梯度不消失
        log_efficiency = math.log(max(1e-6, energy_efficiency))
        
        # 2. 失败信号 - 使用 log(1-success) 而非 -log(success)
        # 当success≈1时，log(1-success)→-∞ 仍提供梯度信号
        failure_signal = -math.log(max(1e-12, 1.0 - float(consensus_success) + 1e-6))
        # 归一化到合理范围
        failure_signal = min(10.0, failure_signal)
        
        # 3. Pareto几何平均 - 平衡多目标
        # (success * (1-energy) * (1-delay))^(1/3) 是Pareto最优的自然度量
        pareto_geo = math.pow(
            max(1e-12, float(consensus_success)) * 
            max(1e-12, 1.0 - float(energy_cost)) * 
            max(1e-12, 1.0 - float(latency_cost)), 
            1.0 / 3.0
        )
        ...
        edge_penalty = max(0.0, edge_ratio - 1.0)  # 只惩罚超过目标的部分
