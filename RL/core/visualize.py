...
def plot_topology(graph: Dict, partitions: Dict[int, int], out_path: str, dpi: int = 160, max_nodes: Optional[int] = None, metrics: Optional[Dict] = None):
    if plt is None:
        return
...
def plot_logical_topology(nodes: List[int], edges: Set[Tuple[int, int]], partitions: Dict[int, int], 
    ...
    if layout == 'kamada_kawai':
        ...
        pos = nx.shell_layout(G, nlist=shells, scale=2.0)
    else:  # spring (default)
        pos = nx.spring_layout(G, k=1.5/math.sqrt(len(nodes)+1), iterations=100, seed=42, scale=2.0)
    ...
    for agent in agents:
        ...
        # Draw nodes
        ax.scatter(xs, ys, s=120, c=color, edgecolors='white', linewidths=1.5, 
                   zorder=3, alpha=0.9, label=f'Agent {agent}')
        
        # Draw node labels
        for n in agent_nodes:
            x, y = pos[n]
            ax.annotate(str(n), (x, y), fontsize=6, ha='center', va='center', 
